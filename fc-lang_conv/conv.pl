...
sub parseFile($$$) {
    ...
    my $ok;
    my $line = 0;
    my ($first, $second, $tail, $iscomment);
    ...
    foreach $line (@lines) {
        if (length($line) > 0) {
            if ($line =~ m/^include\s+(.*)$/) {
                ...
                $second = 0;
                $iscomment = undef;
                if ($line =~ m/^(0x)*([0-9a-fA-F]+)\s*-\s*(0x)*([0-9a-fA-F]+)(.*)$/) {
                    # line contains range with optional comment
                    $first = hex($2);
                    $second = hex($4);
                    $tail = $5;
                    $ok = 1;
                    ...
                    # line contains one number with optional comment
                    $first = hex($2);
                    $tail = $3;
                    $ok = 1;
                } elsif ($line =~ m/^\/\/.*$/) {
                ...
                if ($ok) {
                    if ($iscomment) {
                        ...
                        if (0 == $second) {
                            printf $fout ("\t0x%04x,${tail}\n", $first);
                            $count++;
                        } else {
                            printf $fout ("\t0x%08x, 0x%04x, 0x%04x,	// range${tail}\n", CODE_IN_RANGE, $first, $second);
                            $count += 3;
                        }
                    }
                }
